...
def add_product(manufacturing_order, make, model, variant, lora_id, serial_number, hardware_version, hardware_batch,
    ...
    if(lora_id == None):
        lora_id_for_db = get_next_serial(c)
        lora_id_hex = format(lora_id_for_db, '08X')  # Convert serial_id to 8-digit hexadecimal

    if(lora_id_hex == None):
        lora_id_hex = lora_id
        lora_id_for_db = lora_id
    ...
    c.execute('''
     ...
     ''', (manufacturing_order, make, model, variant, lora_id_for_db, testing_date_code, serial_number, hardware_version, hardware_batch,
